...
class TestCases(TestCase):
    ...
    def test_ntc_snp_as_ts(self, study_mode: StudyMode=StudyMode.TimeSeries):
        tm0 = time.time()
        ...
        print('Running optimal net transfer capacity for snapshot using time series...')

        # if circuit.time_profile is None:
        #     circuit.format_profiles(index=[circuit.format_profiles(index=[datetime.date.today()])])
    ...
    def test_ntc_snp(self):
        ...
        # set optimal net transfer capacity driver instance
        driver = OptimalNetTransferCapacitySnapshotDriver(
            grid=circuit,
            options=options,
