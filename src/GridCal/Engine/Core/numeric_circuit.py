...
def compile_series_circuit(
        circuit: MultiCircuit,
        apply_temperature=False,
        branch_tolerance_mode=BranchImpedanceMode.Specified,
        use_stored_guess=False,
        from_time_series=True,
        opf=True,
) -> SeriesData:
    """
    ...
    :param from_time_series: booelan to indicate where to pick data (snapshot or time series)
    :param opf:
    ...
    if circuit.time_profile is None:
        circuit.format_profiles(
            index=[circuit.format_profiles(
                index=[datetime.date.today()]
            )]
        )

    if from_time_series:
        circuit.time_array = circuit.time_profile
    else:
        circuit.time_array = [datetime.datetime.now()]
    ...
    nc.static_generator_data = gc_compiler.get_static_generator_data(
        circuit=circuit,
        bus_dict=bus_dict,
        time_series=from_time_series,
        ntime=ntime,
    ...
    nc.line_data = gc_compiler.get_line_data(
        circuit=circuit,
        bus_dict=bus_dict,
    ...
    nc.transformer_data = gc_compiler.get_transformer_data(
        circuit=circuit,
        bus_dict=bus_dict,
        time_series=from_time_series,
        ntime=ntime,
    ...
    nc.vsc_data = gc_compiler.get_vsc_data(
        circuit=circuit,
        bus_dict=bus_dict,
        time_series=from_time_series,
        ntime=ntime,
    ...
    nc.upfc_data = gc_compiler.get_upfc_data(
        circuit=circuit,
        bus_dict=bus_dict,
        time_series=from_time_series,
        ntime=ntime,
    ...
    nc.dc_line_data = gc_compiler.get_dc_line_data(
        circuit=circuit,
        bus_dict=bus_dict,
        apply_temperature=apply_temperature,
        branch_tolerance_mode=branch_tolerance_mode,
